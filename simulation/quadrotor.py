...
class Quadrotor_v0(object):
    ...
    def __init__(self, dt):
        ...
        self.obs_low = np.array([-20, -20, -10, -np.pi, -np.pi, -np.pi, -10, -10, -10])
        self.obs_high = np.array([20, 20, 10, np.pi, np.pi, np.pi, 10, 10, 10])
        #
        self.reset()
        # self._t = 0.0
    ...
    def reset(self):
        ...
        self._state[kPosX] = 0.0
        self._state[kPosY] = 0.0
        self._state[kPosZ] = 1.0
    ...
    def _quatToEuler(quat):
        ...
        euler_x = np.arctan2(2*quat_w*quat_x + 2*quat_y*quat_z, quat_w*quat_w - quat_x*quat_x - quat_y*quat_y + quat_z*quat_z)  #  0
        euler_y = -np.arcsin(2*quat_x*quat_z - 2*quat_w*quat_y)  # 0
